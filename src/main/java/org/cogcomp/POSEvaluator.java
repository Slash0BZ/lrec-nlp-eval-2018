...
public class POSEvaluator {
    ...
    public static List<Pair<List<String>, List<String>>> readData(String filename){
        ...
        System.out.println("Read " + sentCount + " Sentences.");
        System.out.println("Read " + tokCount + " Tokens.");
        return ret;
    }
    ...
    public static void transformData(String path, String targetPath) throws IOException{
        List<Pair<List<String>, List<String>>> sentences = readData(path);
        BufferedWriter writer = new BufferedWriter(new FileWriter(targetPath));
        for (Pair<List<String>, List<String>> sentence : sentences){
            String line = "";
            for (int i = 0; i < sentence.getSecond().size(); i++){
                line += sentence.getSecond().get(i) + "_" + sentence.getFirst().get(i) + " ";
            }
    ...
    public static void trainCoreNLP() throws Exception{
        String[] args = new String[2];
        args[0] = "-props";
        args[1] = "CoreNLPModels/train.prop";
        MaxentTagger.main(args);
    }

    public static void testTrainedCoreNLP(){
        MaxentTagger tagger = new MaxentTagger("CoreNLPModels/00-21.tagger");
        List<Pair<List<String>, List<String>>> sentences = readData(TestFile);
        Map<String, Integer> goldMap = new HashMap<>();
        Map<String, Integer> predictMap = new HashMap<>();
        Map<String, Integer> correctMap = new HashMap<>();
        for (Pair<List<String>, List<String>> sent : sentences){
            String sentString = "";
            for (String tok : sent.getSecond()){
                sentString += tok + " ";
            }
            sentString = sentString.substring(0, sentString.length() - 1);
            String tagged = tagger.tagTokenizedString(sentString);
            String[] taggerToks = tagged.split(" ");
            if (taggerToks.length != sent.getFirst().size()){
                System.out.println("error occurred");
                System.exit(0);
            }
            for (int i = 0; i < taggerToks.length; i++){
                String predictedTag = taggerToks[i].split("_")[1];
